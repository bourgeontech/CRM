...
import kotlinx.android.synthetic.main.row_lead.view.tvMobile
import kotlinx.android.synthetic.main.row_lead.view.tvName
import kotlinx.android.synthetic.main.row_lead_two.view.*


class FolloUpLeadAdapter(
    val itemsCells: List<FollowUpList?>,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return if (viewType == Constant.VIEW_TYPE_ITEM) {
            val view = LayoutInflater.from(parent.context)
                .inflate(com.git.crm.R.layout.row_lead_two, parent, false)
            ItemViewHolder(view)
        } else {
            val view =
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder.itemViewType == Constant.VIEW_TYPE_ITEM) {
           // holder.itemView.tvDate.text = itemsCells?.get(position)?.createdDate
               try {
                   holder.itemView.tvName.text = itemsCells?.get(position)?.heading
                   holder.itemView.tvCreatedTime.text =
                       "Created Time : " + parseDateToddMMyyyy2(itemsCells?.get(position)?.Created_Date)
                   holder.itemView.tvDate.text =
                       "Created Date : " + parseDateToddMMyyyy(itemsCells?.get(position)?.Created_Date)
                   holder.itemView.tvTime.text =
                       "Remind Time : " + itemsCells?.get(position)?.remindTime
                   holder.itemView.tvMobile.text = "Remark : " + itemsCells?.get(position)?.remark
                   holder.itemView.tvDateTwo.text =
                       "Remind Date : " + parseDateToddMMyyyy(itemsCells?.get(position)?.remindDate)
                   holder.itemView.tvStatus.text =
                       "Lead Status : " + itemsCells?.get(position)?.Lead_Status
               }catch (e:Exception){
