...
class UserDialogAdapter(
    var activity: FragmentActivity?,
    val fragmentManager: FragmentManager?,
    val doctorlist: List<UserList>

) : RecyclerView.Adapter<UserDialogAdapter.BusListDialogViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BusListDialogViewHolder, position: Int) {
        ...
        holder.txtName.text = doctorlist.get(position).firstName
...
//
        holder.txtName.setOnClickListener {
            ...
            onBusdetailsval.onBus(
                    doctorlist.get(position).userCode!!.toString(),doctorlist.get(position).firstName!!)
