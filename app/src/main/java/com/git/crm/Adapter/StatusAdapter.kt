...
class StatusAdapter(
    val itemsCells: List<StatusList>?,
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder.itemViewType == Constant.VIEW_TYPE_ITEM) {
           // holder.itemView.tvDate.text = itemsCells?.get(position)?.createdDate
            holder.itemView.tvName.text ="Lead Status : "+ itemsCells?.get(position)?.leadStatus
            holder.itemView.tvDate.text ="Added Date : "+itemsCells?.get(position)?.createdDate
            holder.itemView.tvRemark.text ="Remark : "+itemsCells?.get(position)?.Remark
