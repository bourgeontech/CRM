...
class PlanDialogAdapter(
    var activity: FragmentActivity?,
    val fragmentManager: FragmentManager?,
    val doctorlist: List<PlanToBuyList>

) : RecyclerView.Adapter<PlanDialogAdapter.BusListDialogViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BusListDialogViewHolder, position: Int) {
        ...
        holder.txtName.text = doctorlist.get(position).plans
...
//
        holder.txtName.setOnClickListener {
            ...
            onBusdetailsval.onBus(
                    doctorlist.get(position).plantoBuyid!!.toString(),doctorlist.get(position).plans!!)
