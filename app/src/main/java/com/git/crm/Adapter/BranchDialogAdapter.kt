...
class BranchDialogAdapter(
    var activity: FragmentActivity?,
    val fragmentManager: FragmentManager?,
    val doctorlist: List<BranchList>

) : RecyclerView.Adapter<BranchDialogAdapter.BusListDialogViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BusListDialogViewHolder, position: Int) {
        ...
        holder.txtName.text = doctorlist.get(position).branch
...
//
        holder.txtName.setOnClickListener {
            ...
            onBusdetailsval.onBus(
                    doctorlist.get(position).branchId!!.toString(),doctorlist.get(position).branch!!)
