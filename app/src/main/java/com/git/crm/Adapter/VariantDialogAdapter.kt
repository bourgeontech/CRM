...
class VariantDialogAdapter(
    var activity: FragmentActivity?,
    val fragmentManager: FragmentManager?,
    val doctorlist: List<VariantList>

) : RecyclerView.Adapter<VariantDialogAdapter.BusListDialogViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BusListDialogViewHolder, position: Int) {
        ...
        holder.txtName.text = doctorlist.get(position).variant
...
//
        holder.txtName.setOnClickListener {
            ...
            onBusdetailsval.onBus(
                    doctorlist.get(position).variantId!!.toString(),doctorlist.get(position).variant!!)
