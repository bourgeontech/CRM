...
class SubSourceAdapter(
    var activity: FragmentActivity?,
    val fragmentManager: FragmentManager?,
    val doctorlist: List<SubSourceList>

) : RecyclerView.Adapter<SubSourceAdapter.BusListDialogViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BusListDialogViewHolder, position: Int) {
        ...
        holder.txtName.text = doctorlist.get(position).subSource
...
//
        holder.txtName.setOnClickListener {
            ...
            onBusdetailsval.onBus(
                    doctorlist.get(position).subSourceId!!.toString(),doctorlist.get(position).subSource!!)
