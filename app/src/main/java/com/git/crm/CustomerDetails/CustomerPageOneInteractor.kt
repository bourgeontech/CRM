...
class CustomerPageOneInteractor {
    ...
    interface OnLoginFinishedListener {
        ...
        fun onValiSuccess(msg: String, data: List<LeadDetails>)
        fun onValidFailure(msg: String)
        fun onEmailSuccess(msg: String, data: List<LeadDetails>)
    ...
    fun valid(s: String, field: String, listener: OnLoginFinishedListener) {
        val jsonParams: MutableMap<String?, Any?> = ArrayMap()
        jsonParams["Value"] = s
        jsonParams["Field"] = field
        val body = RequestBody.create(
            MediaType.parse("application/json; charset=utf-8"),
            JSONObject(jsonParams).toString()
        )

        val call: Call<SendPojoTwo> = ApiClient.getClient.valid(body)
        call.enqueue(object : Callback<SendPojoTwo> {
            override fun onFailure(call: Call<SendPojoTwo>, t: Throwable) {
                listener.onFailure("There is some problem.Try again")
            }

            override fun onResponse(
                call: Call<SendPojoTwo>,
                response: Response<SendPojoTwo>
            ) {

                if (response.isSuccessful) {
                    if (response.body()?.status!!) {
                        if (field.equals("Mobile"))
                            listener.onValiSuccess(response.body()?.message!!,response?.body()?.data!!)
                        else
                            listener.onEmailSuccess(response.body()?.message!!,response?.body()?.data!!)
                    } else {
                        if (field.equals("Mobile"))
                            listener.onValidFailure(response.body()?.message!!)
                        else listener.onEmailFailure(response.body()?.message!!)
                    }
                } else {
                    listener.onFailure("There is some problem.Try again")
                }
